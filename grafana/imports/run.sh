...
is_valid_health_status() {
    status=`curl -s -u $USER:$PW -H "Content-Type: application/json" \
        -X GET http://$HOSTNAME:$PORT/api/health \
...
curl -s -i -u $USER:$PW -H "Content-Type: application/json" \
    -X POST http://$HOSTNAME:$PORT/api/datasources \
    -d '{"name":"elasticsearch","type":"elasticsearch","url":"http://'$ELASTIC_HOSTNAME':9200", "access":"proxy", "database":"logstash-*","jsonData":{"timeField":"datetime"}}' \
    --retry $RETRY_COUNT > /dev/null

# for index docker
curl -s -i -u $USER:$PW -H "Content-Type: application/json" \
    -X POST http://$HOSTNAME:$PORT/api/datasources \
    -d '{"name":"elasticsearch-docker","type":"elasticsearch","url":"http://'$ELASTIC_HOSTNAME':9200", "access":"proxy", "database":"docker","jsonData":{"timeField":"@timestamp"}}' \
    --retry $RETRY_COUNT > /dev/null

# import storage monitoring dashboard
curl -s -i -u $USER:$PW -H "Content-Type: application/json" \
    -X POST http://$HOSTNAME:$PORT/api/dashboards/import \
...
curl -s -i -u $USER:$PW -H "Content-Type: application/json" \
    -X POST http://$HOSTNAME:$PORT/api/dashboards/import \
    -d '{"overwrite":true,"inputs":[{"name":"DS_ELASTICSEARCH-DOCKER","type":"datasource","pluginId":"elasticsearch","value":"elasticsearch-docker"}],"dashboard":'"`cat /docker-logs.json | jq . -c`"'}' \
    --retry $RETRY_COUNT > /dev/null
